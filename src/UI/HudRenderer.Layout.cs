...
namespace Inject0rHUD.UI
{
    internal sealed partial class HudRenderer
    {
        private Rect BuildUnifiedRect(HudSnapshot snapshot, ModConfig cfg)
        {
            ...
            float h = ComputePanelHeight(
                snapshot.Timers.Count,
                snapshot.Durability.Count,
        ...
        {
            float s = cfg.DurabilityScale.Value;
        ...
        private Rect BuildWidgetRect(int x, int y, float scale)
        {
            float w = 72f * scale;
            float h = 64f * scale;
            return KeepOnScreen(new Rect(x, y, w, h));
        }

        private Rect BuildShipWidgetRect(ModConfig cfg)
        {
            float s = cfg.Scale.Value;
            return KeepOnScreen(new Rect(cfg.ShipPosX.Value, cfg.ShipPosY.Value, 230f * s, 132f * s));
        }

        private Rect BuildTimeWidgetRect(ModConfig cfg)
        {
            float s = cfg.Scale.Value;
            return KeepOnScreen(new Rect(cfg.TimePosX.Value, cfg.TimePosY.Value, 190f * s, 88f * s));
        }
            ...
            float scale)
        {
            ...
            if (!separate && timers > 0 && durability > 0)
                h += gap;

            if (rows == 0)
                h = Mathf.Max(h, 58f * scale);
        ...
        private Rect BuildSettingsRect(Rect anchor)
        {
            ...
            if (x + width > Screen.width - edge)
                x = anchor.xMin - gap - width;

            if (x < edge)
                x = Mathf.Max(edge, Screen.width - width - edge);
            ...
            ref Rect time)
        {
            ...
            {
                if (settings.Contains(e.mousePosition))
                    return;

                EditTarget target = HitTarget(cfg, e.mousePosition, unified, timers, durability, fps, ping, ship, time);
                if (target == EditTarget.None)
                    return;

                _target = target;
                Rect rect = GetRect(target, unified, timers, durability, fps, ping, ship, time);
                _resizeMode = IsMetric(target) ? ResizeMode.Move : HitResizeMode(rect, e.mousePosition, 8f);

                _dragStart = e.mousePosition;
                _dragRectStart = rect;
                _dragScaleStart = GetTargetScale(target, cfg);
                e.Use();
                return;
            }
        ...
        private static bool IsMetric(EditTarget target)
        {
            return target == EditTarget.Fps || target == EditTarget.Ping ||
                   target == EditTarget.Ship || target == EditTarget.Time;
        }

        private EditTarget HitTarget(
            ModConfig cfg,
            Vector2 mouse,
            Rect unified,
            Rect timers,
            ...
            Rect time)
        {
            if (Expanded(time, 7f).Contains(mouse)) return EditTarget.Time;
            if (Expanded(ship, 7f).Contains(mouse)) return EditTarget.Ship;
            ...
            {
                if (Expanded(unified, 7f).Contains(mouse)) return EditTarget.Unified;
            }
            ...
            Rect time)
        {
            switch (target)
            {
                ...
                case EditTarget.Ship: return ship;
                case EditTarget.Time: return time;
        ...
        private void ApplyResize(ref Rect rect, Vector2 delta, ModConfig cfg)
        {
            ...
            if (top || bottom)
            {
                ...
                float newScale = Mathf.Clamp(_dragScaleStart * ratio, 0.70f, 1.60f);
                SetTargetScale(_target, cfg, newScale);

                if (top)
                    rect.y = _dragRectStart.yMax - (_dragRectStart.height * (newScale / Mathf.Max(0.01f, _dragScaleStart)));
            }
        ...
        private void ApplyRectToConfig(EditTarget target, Rect rect, ModConfig cfg)
        {
            switch (target)
            {
                case EditTarget.Timers:
                    cfg.TimersPosX.Value = Mathf.RoundToInt(rect.x);
                    cfg.TimersPosY.Value = Mathf.RoundToInt(rect.y);
                    cfg.TimersWidth.Value = Mathf.Clamp(
                        Mathf.RoundToInt(rect.width / Mathf.Max(0.01f, cfg.TimersScale.Value)), 120, 720);
                    break;

                case EditTarget.Durability:
                    cfg.DurabilityPosX.Value = Mathf.RoundToInt(rect.x);
                    cfg.DurabilityPosY.Value = Mathf.RoundToInt(rect.y);
                    cfg.DurabilityWidth.Value = Mathf.Clamp(
                        Mathf.RoundToInt(rect.width / Mathf.Max(0.01f, cfg.DurabilityScale.Value)), 120, 720);
                    break;

                case EditTarget.Fps:
                    cfg.FpsPosX.Value = Mathf.RoundToInt(rect.x);
                    cfg.FpsPosY.Value = Mathf.RoundToInt(rect.y);
                    break;

                case EditTarget.Ping:
                    cfg.PingPosX.Value = Mathf.RoundToInt(rect.x);
                    cfg.PingPosY.Value = Mathf.RoundToInt(rect.y);
                    break;

                case EditTarget.Ship:
                    cfg.ShipPosX.Value = Mathf.RoundToInt(rect.x);
                    cfg.ShipPosY.Value = Mathf.RoundToInt(rect.y);
                    break;

                case EditTarget.Time:
                    cfg.TimePosX.Value = Mathf.RoundToInt(rect.x);
                    cfg.TimePosY.Value = Mathf.RoundToInt(rect.y);
                    break;

                default:
                    cfg.PosX.Value = Mathf.RoundToInt(rect.x);
                    cfg.PosY.Value = Mathf.RoundToInt(rect.y);
                    cfg.PanelWidth.Value = Mathf.Clamp(
                        Mathf.RoundToInt(rect.width / Mathf.Max(0.01f, cfg.Scale.Value)), 120, 720);
                    break;
            }
        }

        private static float GetTargetScale(EditTarget target, ModConfig cfg)
        {
            if (target == EditTarget.Timers) return cfg.TimersScale.Value;
            if (target == EditTarget.Durability) return cfg.DurabilityScale.Value;
            return cfg.Scale.Value;
        }

        private static void SetTargetScale(EditTarget target, ModConfig cfg, float value)
        {
            value = Mathf.Clamp(value, 0.70f, 1.60f);
            if (target == EditTarget.Timers) cfg.TimersScale.Value = value;
            else if (target == EditTarget.Durability) cfg.DurabilityScale.Value = value;
            else cfg.Scale.Value = value;
        }

        private void SnapMove(ref Rect r, ModConfig cfg)
        {
            if (!cfg.SnapEnabled.Value)
            {
                r = KeepOnScreen(r);
                return;
            }
        ...
        {
            bool left = Mathf.Abs(mouse.x - r.xMin) <= edge;
            bool right = Mathf.Abs(mouse.x - r.xMax) <= edge;
            bool top = Mathf.Abs(mouse.y - r.yMin) <= edge;
            bool bottom = Mathf.Abs(mouse.y - r.yMax) <= edge;
        ...
        private void DrawEditorChrome(Rect r, string title)
        {
            ...
            float h = 7f;
            DrawHandle(r.xMin, r.yMin, h);
            DrawHandle(r.xMax, r.yMin, h);
            DrawHandle(r.xMin, r.yMax, h);
            DrawHandle(r.xMax, r.yMax, h);
